...
export default (editor: LexicalEditor, selection: RangeSelection, table_node: TableNode) => {
	...
	if (!dom_selection || dom_selection.anchorNode !== editor.getRootElement()) return undefined

	const anchor_cell_node = $findMatchingParent(selection.anchor.getNode(), n =>
		$isTableCellNode(n)
	) as TableCellNode

	if (!anchor_cell_node) return undefined

	const parent_table = $findMatchingParent(anchor_cell_node, n => $isTableNode(n))

	if (!$isTableNode(parent_table) || !parent_table.is(table_node)) return undefined
