...
export function getEndOfCodeInLine(anchor: CodeHighlightNode | TabNode) {
	return getLastCodeNodeOfLine(anchor)
}
		...
		() => {
			updateAndRetainSelection(nodeKey, () => {
				...
				const { tokens } = highlighter.codeToTokens(code, {
					lang: currentNode.__lang as any,
					theme: 'github-light'
				})

				const highlightNodes = getHighlightNodes(tokens as any)
...
function getHighlightNodes(tokens: Array<any>, type?: string): LexicalNode[] {
	...
	tokens.forEach((line, index) => {
		line.forEach(token => {
			...
			if (content === '\t') {
				...
				nodes.push($createCodeHighlightNode(content, color))
			}
		})

		if (tokens.at(index + 1)) nodes.push($createLineBreakNode())
	})
...
function $getCodeLines(selection: RangeSelection) {
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if ($isLineBreakNode(node)) {
			...
			lastLine.push(node as CodeHighlightNode | TabNode)
		}
	}
	return lines
}

function handleTab(shiftKey: boolean) {
	const selection = $getSelection()
	...
	if (codeLines.length > 1) {
		for (let i = 0; i < codeLinesLength; i++) {
			...
			if (line.length > 0) {
				...
				if (i === 0) {
					firstOfLine = getFirstCodeNodeOfLine(firstOfLine) as CodeHighlightNode | TabNode
				}
				if (firstOfLine !== null) {
...
function handleMoveTo(type: LexicalCommand<KeyboardEvent>, event: KeyboardEvent): boolean {
	...
	if (isMoveToStart) {
		...
		const node = getEndOfCodeInLine(focusNode) as CodeHighlightNode
		node.select()
	}
...
export function registerCodeHighlighting(editor: LexicalEditor): () => void {
	...
	return mergeRegister(
		...
		editor.registerNodeTransform(CodeNode, node => codeNodeTransform(node, editor)),
		editor.registerNodeTransform(TextNode, node => textNodeTransform(node, editor)),
		editor.registerNodeTransform(CodeHighlightNode, node => textNodeTransform(node, editor)),
