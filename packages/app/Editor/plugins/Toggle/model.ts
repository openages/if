...
import { INSERT_TOGGLE_COMMAND } from '@/Editor/commands'
...
export default class Index {
	...
	toggleNodeTranform(node: ToggleNode) {
		...
		if (children.length === 2 && $isToggleHeadNode(children[0]) && $isToggleBodyNode(children[1])) return
	...
	insert() {
		this.editor.update(() => {
			...
			const node = $createToggleNode({}).append(
				head.append(p_title),
				$createToggleBodyNode().append(p_body)
			)
