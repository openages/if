...
import { mingoUpdater } from '../../../node_modules/rxdb/src/plugins/update/mingo-updater'
					...
					collection.storageInstance.bulkWrite = async function (writes, context) {
						await Promise.all(
							writes.map(async write => {
								...
								if (
									!config.auto_clean &&
									!deepEqual(docWithoutMeta(newDocState), docWithoutMeta(rebuild))
								) {
								...
								const recalculatedHash = await hashCRDTOperations(
