...
import type { IPropsHomepageDirtree } from '@/layout/types'

const getActions = async (active: IPropsHomepageDirtree['active']) => {
	return match(active)
		.with('todo', async () => {
...
const Index = (props: IPropsHomepageDirtree) => {
	const { active } = props
	...
	return (
		<div className={$cx(styles._local)}>
			<DirTree module={active} actions={actions} use_by_homepage></DirTree>
		</div>
	)
}
